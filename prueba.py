...
class CryptoVaultApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("NexKey Pro - Accesibilidad")
        ...
        # Variable de Accesibilidad (1.0 es el tamaño normal)
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, "r") as f:
                    ...
                    else: self.mostrar_pantalla_registro()
            except: self.mostrar_pantalla_registro()
        else: self.mostrar_pantalla_registro()
    ...
    # --- ACCESIBILIDAD ---
    def cambiar_escala(self, nivel):
        """Ajusta el tamaño de la letra y componentes (1.0, 1.2, 1.5)"""
        valores = {"Pequeño": 0.8, "Normal": 1.0, "Grande": 1.2, "Extra Grande": 1.5}
        nueva_escala = valores.get(nivel, 1.0)
        self.escala_actual = nueva_escala
        ctk.set_widget_scaling(nueva_escala) # Comando clave de accesibilidad
        self.update()
    ...
    def mostrar_pantalla_registro(self):
        ...
        ctk.CTkLabel(frame, text="🛡️ Registro de Bóveda", font=("Helvetica", 26, "bold"), text_color=COLOR_ACCENTO).pack(pady=20)
        ...
        ctk.CTkButton(frame, text="ACTIVAR CIFRADO", fg_color=COLOR_EXITO, height=50, width=300, 
                      command=self.guardar_registro).pack(pady=30)

    def guardar_registro(self):
        u, n, b = self.reg_user.get(), self.reg_nfc.get(), self.reg_bio.get()
        if u and n and b:
            rec = ''.join(random.choices(string.digits, k=6))
            with open(self.config_file, "w") as f: f.write(f"{u}|{n}|{b}|{rec}")
            messagebox.showinfo("GUARDAR CÓDIGO", f"Tu código de emergencia es: {rec}")
            self.verificar_estado_inicial()
        else: messagebox.showwarning("Error", "Completa todos los datos.")
    ...
    def mostrar_pantalla_acceso(self):
        ...
        ctk.CTkButton(frame, text="DESCIFRAR Y ENTRAR", fg_color=COLOR_ACCENTO, height=50, width=300, 
                      command=self.verificar_acceso).pack(pady=20)
        ctk.CTkButton(frame, text="¿Perdiste tu NFC?", fg_color="transparent", text_color=COLOR_PELIGRO, command=self.recuperar).pack()

    def verificar_acceso(self):
        if self.acc_nfc.get() == self.nfc_id and self.acc_bio.get() == self.bio_pin:
            self.mostrar_dashboard()
        else: messagebox.showerror("Error", "Llaves incorrectas.")

    def recuperar(self):
        d = ctk.CTkInputDialog(text="Introduce código de 6 dígitos:", title="Recuperación")
        if d.get_input() == self.recovery_code: self.mostrar_dashboard()
    ...
    def mostrar_dashboard(self):
        ...
        ctk.CTkButton(sidebar, text="⚙️ Ajustes", fg_color="transparent", command=self.mostrar_configuracion).pack(fill="x", padx=10)
        ...
        header.pack(fill="x")
        ctk.CTkLabel(header, text=f"Bóveda de {self.user_name}", font=("Helvetica", 24, "bold")).pack(side="left")
    ...
    def actualizar_lista(self):
        ...
        for arch in os.listdir(self.vault_folder):
            ...
            ctk.CTkLabel(f, text=f"🔒 {arch[:30]}").pack(side="left", padx=15)
    ...
    def subir_y_cifrar(self):
        ...
        if ruta:
            try:
                ...
                with open(os.path.join(self.vault_folder, os.path.basename(ruta)), "wb") as f:
                    f.write(datos_cifrados)
                self.actualizar_lista()
            except: messagebox.showerror("Error", "No se pudo cifrar.")

    def abrir_y_descifrar(self, nombre):
        try:
            fernet = self.obtener_llave_maestra()
            with open(os.path.join(self.vault_folder, nombre), "rb") as f: datos = f.read()
            datos_claros = fernet.decrypt(datos)
            temp = os.path.join(os.environ['TEMP'], nombre)
            with open(temp, "wb") as f: f.write(datos_claros)
            os.startfile(temp) if os.name == 'nt' else os.system(f'open "{temp}"')
        except: messagebox.showerror("Error", "Llave inválida.")
    ...
    def mostrar_configuracion(self):
        ...
        ctk.CTkLabel(f, text="⚙️ Ajustes de Sistema", font=("Helvetica", 26, "bold"), text_color=COLOR_ACCENTO).pack(pady=20)
        
        # --- SECCIÓN ACCESIBILIDAD ---
        ctk.CTkLabel(f, text="♿ Tamaño de Interfaz", font=("Helvetica", 16, "bold")).pack(pady=(10, 5))
        menu_escala = ctk.CTkOptionMenu(f, values=["Pequeño", "Normal", "Grande", "Extra Grande"], 
                                       command=self.cambiar_escala, fg_color=COLOR_EXITO)
        menu_escala.set("Normal")
        menu_escala.pack(pady=10)
        
        # --- SECCIÓN TEMA ---
        ctk.CTkLabel(f, text="🎨 Apariencia", font=("Helvetica", 16, "bold")).pack(pady=(20, 5))
        menu_tema = ctk.CTkOptionMenu(f, values=["Dark", "Light", "System"], command=self.cambiar_tema, fg_color=COLOR_ACCENTO)
        menu_tema.set(ctk.get_appearance_mode())
        menu_tema.pack(pady=10)

        ctk.CTkLabel(f, text=f"\n👤 Usuario: {self.user_name}\n🔑 NFC ID: {self.nfc_id}\n🆘 Auxiliar: {self.recovery_code}", 
                     justify="left", font=("Helvetica", 14)).pack(pady=20)
        
        ctk.CTkButton(f, text="VOLVER AL DASHBOARD", command=self.mostrar_dashboard, width=250).pack(pady=20)

    def cambiar_tema(self, t):
